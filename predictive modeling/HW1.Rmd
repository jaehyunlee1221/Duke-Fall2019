...
hlgt_gp <- hlgt_gp +
  labs(title = "Relatioship between horsepower and weight",
...
$$ E(Y-g(x))^{2} \mid X=x) = E[(Y-f(x))^{2} \mid X] \\ +
 E[(f(x)-g(x))^{2} \mid X=x] + 2(f(x)-g(x))E[(Y-f(x)] $$
...
(a) $E(Y \mid X=x_{0})$ = $E(f(x_{0} + \varepsilon \mid X=x_{0})$
										 =$f(x_{0})$  
										 
(b) $var(Y \mid X=x_{0})$ = $var(f(x_{0})) + \sigma^{2}$  
  
(c) Let $E(\hat{f}(x_{0})) = m$, Then
...
(e) Like (c\), let $E(\hat{f}(x_{0})$ be $m$. Then,
$$ E[(f(x_{0})-\hat{f}(x_{0})^{2}] = E[(f(x_{0}) - m + m - \hat{f}(x_{0})^{2}] \\ $$ 
$$
= E[(f(x_{0}) - m)^{2}] + E[(\hat{f}(x_{0})-m)^{2}] + (f(x_{0})-m)E[(\hat{f}(x_{0})-m)] \\ = var(\hat{f}(x_{0})) + Bias(\hat{f}(x_{0}))^{2}$$
