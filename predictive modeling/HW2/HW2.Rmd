...
As we can see at above plot, value 1 is included in 95% confidence interval of lambda which is same result from Q6. Thus we don't need to transform  However, even though we have checked through 'powerTransform' function, we should check again with boxcox function
...
par(mfrow=c(2,2))
plot(Cm, 
     sub.caption = "fig10. every plot don't show any violation of assumption")
...
When executing outlier test with Bonferonni correction, I cannot find any outliers. Even though there are some suspects for outlier, they leverage changed after taking power transform.
...
interpret <- c("When predictor variables are 0, ModernC has -17.879 on average","Increase in 1 unit of Change_add makes increase 2.31 unit of ModernC on average","Increase in 1 unit of PPgdp makes increase 6.44*log(1+1/PPgdp) unit of ModernC on average","Increase in 1 unit of Frate makes increase 0.17 unit of ModernC on average","Increase in 1 unit of Pop makes increase 1.59*log(1+1/Pop) unit of ModernC on average","Increase in 1 unit of Fertility makes decrease -18*log(1+1/Fertility) unit of ModernC on average","Increase in 1 unit of Purban makes decrease 0.007 unit of ModernC on average")
...
$$
\hat{\beta_{0}} = \bar{e_{Y}} - \hat{\beta_{p}} \bar{e_{x_{p}}}
$$
and
$$
\bar{e_{Y}} = \sum_{i=1}^{n} e_{Y_{i}}/n = 1/n * 1_{n}^{T}(I-H^{*})Y
$$
where $H^{*}=X^{*}(X^{*}X^{*})^{-1}X^{*T}$.
But $H^{*}$ is projection matrix of X which include one's column. According to fact that if $H^*$ is the projection matrix for $X$ which contains a column of ones, then $1_{n}^{T} (I - H) = 0$ or $(I - H) 1_{n} = 0$, $\bar{e_{Y}}$ = 0
$$
\bar{e_{x_{p}}} = \sum_{i=1}^{n} e_{X_{pi}}/n = 1/n * 1_{n}^{T}(I-H^{*})x_{p}
$$
By same logic, $\bar{e_{x_{p}}}$ = 0.
